...
# 로그인 후 접근할 페이지
search_url = "https://www.jobplanet.co.kr/search?query="
...
for corp in corporate_list:
    # 1. 회사 기본 페이지 요청
    res = safe_request(session, 'GET', search_url + corp, headers=headers)
    if not res:
        print("=" * 50)
        print("last cid:", corp, "line: 106")
        break # 요청 실패 시 다음 ID로 건너뜀
    soup = BeautifulSoup(res.text, 'html.parser')
    company_url = soup.select_one("#contentsWrap > div.mx-auto > div > div:nth-child(1) > div > div > ul > a.group.desktop:nth-child(1)")
    company_url = company_url.get("href")
    res = safe_request(session, 'GET', company_url, headers=headers)
    if "landing" in res.url:
        res = safe_request(session, 'GET', res.url.replace("landing", "reviews"), headers=headers)
    if not res:
        print("=" * 50)
        print("last cid:", i, "line: 115")
        break # 요청 실패 시 다음 ID로 건너뜀

    soup = BeautifulSoup(res.text, 'html.parser')
    if soup.select_one("#premiumReviewStatistics"):
        company_name = unquote(urlparse(res.url).path).strip('/').split('/')[-1]
        cid = unquote(urlparse(res.url).path).strip('/').split('/')[-3]
        ...
        filename = os.path.join(dir_name, f"{clean_company_name}_{cid}_jobplanet.jsonl")
        with open("./jobplanet_company_name_mapping.txt", 'a+', encoding='utf-8') as f:
            f.write(f"{corp}, {company_name}, {cid}, {company_url}\n")
        
        year = ""
        # 2. 첫 번째 API 요청 (전체 카운트 확인)
        res = safe_request(session, 'GET', api_url.format(cid, 1, year), headers=headers)
        if not res: 
            print("=" * 50)
            print("last cid:", cid, "line: 135")
            break
        ...
        if count > 0:
            ...
            with open(filename, 'a+', encoding='utf-8') as f:
                for page in tqdm(range(1, int(max_page) + 1)): # range 범위 정상화 (+1 추가)
                    ...
                    res = safe_request(session, 'GET', api_url.format(cid, page, year), headers=headers)
                    if not res: 
                        print("=" * 50)
                        print("last cid:", cid, "page:", page, "line: 119")
                        break
        ...
        with open("./jobplanet_error.txt", 'a+', encoding='utf-8') as f:
            f.write(f"{corp}, {cid}, {res.url}\n")
        print("last cid:", cid, "line 188")
        continue